...
DSpotify::~DSpotify() {
    // Break circular references by clearing all genre references from songs
    // and clearing all root references from genres

    // First, clear all genre pointers from songs to break the cycles
    for (int i = 0; i < songs.getCapacity(); ++i) {
        auto bucket = songs.getBucket(i);
        if (bucket) {
            auto current = bucket->getStart();
            while (current) {
                auto song = current->getData();
                if (song) {
                    song->setGenre(nullptr);  // Break the cycle
                    song->setFather(nullptr); // Clear father references
                }
    ...
    // Then clear all root pointers from genres
    for (int i = 0; i < genres.getCapacity(); ++i) {
        auto bucket = genres.getBucket(i);
        if (bucket) {
            auto current = bucket->getStart();
            while (current) {
                auto genre = current->getData();
                if (genre) {
                    genre->setRoot(nullptr);  // Break the cycle
                }
                current = current->getNext();
            }
        }
...
StatusType DSpotify::addSong(int songId, int genreId) {
    ...
    if (!genres.find(genreId)) { /// check if need to add other checks
    ...
    shared_ptr<Genre> genre = genres.find(genreId); // Get the genre from the hash table
...
StatusType DSpotify::mergeGenres(int genreId1, int genreId2, int genreId3) {
    ...
    if (!genres.find(genreId1) || !genres.find(genreId2)) {
    ...
    shared_ptr<Genre> genre1 = genres.find(genreId1);

    shared_ptr<Genre> genre2 = genres.find(genreId2);
