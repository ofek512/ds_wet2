...
private:
    ...
    long A, B;
    static constexpr long P = 2147483647; // A large prime number for hashing
...
public:
    ...
    Hash() : curr_size(0), insert_counter(0), prime_index(0),
        ...
        A = (std::rand() % (P - 1)) + 1;            // pick A ∈ [1, P−1]
        B = std::rand() % P;                        // pick B ∈ [0, P−1]
    }
...
hashingResult<T> Hash<T>::hash_insert(const T &data) {
    ...
    // hk(x) = (h(x) + k * r(x)) % m
    // Primary hash:
    long h1 = ((A * key + B) % P) % max_size;
    // Secondary step:
    long h2 = (key % (max_size - 1)) + 1;
    if (h2 <= 0) h2 = 1;
...
hashingResult<T> Hash<T>::hash_search(const T &data) {
    ...
    long k = 0;
    int attempts = 0;
    // First step of double hashing:
    long h1 = ((A * key + B) % P) % max_size;
    // Secondary step:
    long h2 = (key % (max_size - 1)) + 1;
    while (attempts < max_size) {
        long index = (h1 + k * h2) % max_size;
        if (index < 0) index += max_size;
