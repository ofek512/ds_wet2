...
// However, you need to implement all public DSpotify functions, as provided below as a template
...
StatusType DSpotify::addSong(int songId, int genreId) {
    ...
    // Create a song node and add it to the hash table and then to the union member
...
StatusType DSpotify::mergeGenres(int genreId1, int genreId2, int genreId3) {
    ...
    // Handle an empty genres case
    shared_ptr<Song> root1 = genre1->getRoot();
    shared_ptr<Song> root2 = genre2->getRoot();
    if (!root1 && !root2) {
        // Both genres are empty - insert a new empty genre
    } else if (!root1) {
        // Only genre2 has songs
        newGenre->setRoot(root2);
        root2->setGenre(newGenre);  // Update genre pointe
        ...
        if (genre1->getSize() >= genre2->getSize()) {
            // attach a smaller tree (root2) under root1
...
output_t<int> DSpotify::getNumberOfGenreChanges(int songId) {
    ...
    // Get the song from the hash table
    shared_ptr<Song> song = songs.member(songId);
    // Compress the path to member the root song and then simply member Song again
...
shared_ptr<Song> DSpotify::findSet(shared_ptr<Song> song) {
    ...
    // First member the root without compression
